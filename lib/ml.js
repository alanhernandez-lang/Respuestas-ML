...
// Igual que fetchPackMessages: este endpoint también pagina (default de 10 por
// página, documentado en ML) y sin `limit`/`offset` solo llegaba la primera —
// cualquier pack "no leído" que sobrara de esos 10 quedaba invisible para el sync,
// y como el resto del código asume unreadCount=0 para todo lo que no aparezca aquí,
// esos packs terminaban marcados como "leídos" aunque en realidad siguieran sin
// leer (y sin contestar). Pedimos páginas grandes y juntamos todo lo que reporte
// `paging.total`, igual que ya hacíamos para el historial de mensajes.
async function fetchUnreadPacks(token) {
  const PAGE_SIZE = 50;
  const PAGE_GUARD = 20;
  const path = (offset) => `/messages/unread?role=seller&tag=post_sale&limit=${PAGE_SIZE}&offset=${offset}`;

  const first = await mlFetch(token, path(0));
  let results = first.results || [];
  const total = first.paging?.total ?? results.length;
  const pageSize = first.paging?.limit || PAGE_SIZE;

  let guard = 0;
  while (results.length < total && guard < PAGE_GUARD) {
    guard++;
    const next = await mlFetch(token, `/messages/unread?role=seller&tag=post_sale&limit=${pageSize}&offset=${results.length}`);
...
// 2026-08-29: solo hace falta después de perder el caché completo (Redis se quedó
// sin cupo y hubo que empezar de cero) — a diferencia de fetchUnreadPacks (que solo
// trae lo NO leído), esto trae TODAS las ventas del vendedor, para poder reconstruir
// también las conversaciones ya respondidas. sort=date_asc a propósito: así el orden
// en el que se procesan coincide aproximadamente con el orden cronológico real,
// para que la Bitácora (que no se reordena al mostrarse, ver /api/log) quede lo más
// parecida posible al orden real aunque el backfill corra con varios packs a la vez.
//
// /orders/search de Mercado Libre no deja paginar más allá de offset+limit > 10000
// en total (tienda con años de historial, muchas más de 10000 órdenes en su vida) —
// como esta app solo necesita las ventas de la ventana real donde ha estado
// operando (semanas/meses, no años), se acota con order.date_created.from en vez de
// tronar al llegar a ese tope.
async function fetchAllSellerOrders(token, sellerId, daysBack = 180) {
  const from = new Date(Date.now() - daysBack * 24 * 60 * 60 * 1000).toISOString();
  const PAGE_SIZE = 50;
  const OFFSET_HARD_CAP = 10000;
  let offset = 0;
  let all = [];
  let total = Infinity;
  while (offset < total && offset < OFFSET_HARD_CAP) {
    const resp = await mlFetch(
      token,
      `/orders/search?seller=${sellerId}&sort=date_asc&offset=${offset}&limit=${PAGE_SIZE}&order.date_created.from=${encodeURIComponent(from)}`,
    );
    const results = resp.results || [];
    if (!results.length) break;
    all = all.concat(results);
    total = resp.paging?.total ?? all.length;
    offset += PAGE_SIZE;
  }
  return all;
}

// `order.shipping.id` (si existe) apunta a este recurso — de aquí sale si el envío
// es FULL (`logistic_type: "fulfillment"`) y el estatus real (shipped/delivered/...).
// Cuando la venta se acuerda directo con el comprador (sin logística de ML),
// `order.shipping` no trae `id` y este endpoint ni se llama.
function fetchShipmentDetail(token, shipmentId) {
  return mlFetch(token, `/shipments/${shipmentId}`);
}
