...
- Esa plantilla pide teléfono, foto/video y domicilio para PODER EMPEZAR a canalizar el caso. Si el
  ...
  pedir el envío gratis), o ya mandó fotos/video del problema, QUITA esos puntos de la lista —
  jamás los vuelvas a pedir, ya se recibieron.
- Si TODOS los datos que pediría la plantilla ya se dieron (teléfono, evidencia, domicilio si
  aplica) Y/O el vendedor ya respondió reconociendo el caso y prometiendo una acción concreta
  (reenvío, garantía en trámite, etc.): NO uses esta plantilla en absoluto, el caso ya pasó esa
  etapa. En su lugar redacta una respuesta breve que reconozca lo ya hablado y dé una actualización
  honesta del estatus (ver también la regla de "casos YA EN PROCESO" de arriba).
...
// razonamiento cuentan contra el MISMO límite de salida — en prompts largos con
// varias fotos a veces se gasta todo el presupuesto pensando y nunca llega a
// escribir texto (responde 200 OK pero vacío, finishReason "MAX_TOKENS"; lo vimos en
// un caso real). Redactar una respuesta corta no necesita razonamiento extendido,
// así que se desactiva. "-pro" no lo permite en 0 (mínimo 128) y tiraría error si se
// lo mandamos, así que solo se aplica a flash/flash-lite.
function buildGenerationConfig() {
  return GEMINI_MODEL.includes('pro') ? undefined : { thinkingConfig: { thinkingBudget: 0 } };
}
