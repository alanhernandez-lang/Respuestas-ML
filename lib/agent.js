...
async function generateDraftAnswer({ buyerName, itemTitles, messages, token, orderCreationDate, frequentResponses }) {
  ...
  const draftInput = { buyerName, itemTitles, messages, orderCreationDate, frequentResponses };
  const parts = buildContentParts(draftInput, imagesByMessageIndex);
  ...
  if (!result.text && result.blockReason && imagesByMessageIndex.size > 0) {
  ...
  return { text: result.text, imagesExcluded };
}
