...
async function generateDraftAnswer({ buyerName, itemTitles, messages, token, orderCreationDate, frequentResponses }) {
  ...
  // Los modelos 2.5 "piensan" antes de escribir la respuesta visible, y esos tokens
  // de razonamiento cuentan contra el MISMO límite de salida — en prompts largos con
  // varias fotos (como este, con el banco de plantillas completo + imágenes) a veces
  // se gasta todo el presupuesto pensando y nunca llega a escribir texto (responde
  // 200 OK pero vacío, finishReason "MAX_TOKENS"; lo vimos en un caso real). Redactar
  // una respuesta corta no necesita razonamiento extendido, así que se desactiva.
  // "-pro" no lo permite en 0 (mínimo 128) y tiraría error si se lo mandamos, así que
  // solo se aplica a flash/flash-lite.
  const generationConfig = GEMINI_MODEL.includes('pro')
    ? undefined
  ...
  if (!text) {
    ...
    // exacto (bloqueo de seguridad, corte por límite de tokens, etc.) es mucho más
    // fácil saber si hace falta ajustar el prompt o si fue un caso raro y aislado.
    const blockReason = data.promptFeedback?.blockReason;
    const finishReason = candidate?.finishReason;
    const detail = blockReason
      ? `bloqueó la respuesta por seguridad (${blockReason})`
      : finishReason && finishReason !== 'STOP'
        ? `terminó sin texto visible (${finishReason})`
        : 'no devolvió texto de respuesta';
