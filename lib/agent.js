...
async function callGemini(parts, apiKey, extraGenerationConfig) {
...
function buildExtractionPrompt(messages, fieldLabels) {
  const fieldsList = Object.entries(fieldLabels).map(([key, label]) => `- "${key}": ${label}`).join('\n');
  const thread = (messages || [])
    .map((m) => `${m.sender === 'cliente' ? 'Cliente' : 'Vendedor'}: ${m.text || '[archivo adjunto]'}`)
    .join('\n');
  return `Del siguiente hilo de conversación de Mercado Libre, extrae ÚNICAMENTE estos datos, y
ÚNICAMENTE si el CLIENTE los escribió explícitamente en alguno de sus mensajes (nunca un dato que
haya escrito el vendedor, nunca lo inventes ni lo completes por suposición o contexto):
${fieldsList}
...
Cada valor debe ser el texto exacto que escribió el cliente (puedes recortar saludos/relleno
alrededor, pero no reescribas ni corrijas el dato en sí), o \`null\` si el cliente no dio ese dato
específico en ningún mensaje del hilo.
...
async function extractStructuredData(messages, fieldLabels, apiKey) {
  if (!apiKey) throw new Error('Falta configurar GEMINI_API_KEY en .env');
  let result;
  try {
    result = await callGemini([{ text: buildExtractionPrompt(messages, fieldLabels) }], apiKey, {
...
function extractRefacturaData(messages, apiKey) {
  return extractStructuredData(messages, REFACTURA_FIELD_LABELS, apiKey);
}

function extractEnvioAcordadoData(messages, apiKey) {
  return extractStructuredData(messages, ENVIO_ACORDADO_FIELD_LABELS, apiKey);
}
...
CFDI/factura fiscal de este pedido).

Responde que SÍ únicamente si el cliente está solicitando activamente una factura o refactura.
